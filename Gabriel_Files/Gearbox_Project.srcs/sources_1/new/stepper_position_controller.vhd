...
architecture rtl of stepper_position_controller is
    constant HOMING_STEP_HZ   : integer := 400; 
    constant RUN_STEP_HZ      : integer := 500; 
    constant REQ_HOMING_STEPS : integer := 2000; 

    type state_t is (IDLE, CALIB_MOVE, CALIB_WAIT, READY, MOVE);
    signal state : state_t := IDLE;
    type step_state_t is (S0,S1,S2,S3,S4,S5,S6,S7);
    signal step_state : step_state_t := S0;

    signal current_pos : integer := 0;
    signal tick_cnt, wait_cnt, homing_steps : integer := 0;
...
begin
    ...
    begin
        if rising_edge(clk) then
            if rst = '1' then
                state <= IDLE; calibrated_i <= '0';
            else
                case state is
                    when IDLE =>
                        if start_calibration = '1' then
                            state <= CALIB_MOVE; tick_cnt <= 0; homing_steps <= 0;
                        end if;

                    when CALIB_MOVE =>
                        if tick_cnt >= (CLK_FREQ_HZ / HOMING_STEP_HZ) then
                            tick_cnt <= 0;
                            -- Calibration moving 'Up' (Direction 1)
                            act_dir := '1' xor invert_direction; 
                            if act_dir = '1' then
                                case step_state is
                            ...
                            if homing_steps >= REQ_HOMING_STEPS then state <= CALIB_WAIT; wait_cnt <= 0; end if;
                        else tick_cnt <= tick_cnt + 1;
                        end if;

                    when CALIB_WAIT =>
                        if wait_cnt >= (CLK_FREQ_HZ/10) then 
                            -- Once hit, we define the wall as 0. 
                            -- Any positive target will now move "Down" away from the wall.
                            current_pos <= 0; calibrated_i <= '1'; state <= READY;
                        else wait_cnt <= wait_cnt + 1;
                        end if;

                    when READY =>
                        if target_position /= current_pos then state <= MOVE; tick_cnt <= 0; end if;
                    ...
                    when MOVE =>
                        if current_pos = target_position then
                            ...
                            tick_cnt <= 0;
                            if target_position > current_pos then log_dir := '1'; else log_dir := '0'; end if;
                            
                            -- Because Calibration used act_dir '1', moving to a positive 
                            -- target (log_dir '1') will now use act_dir '0'. 
                            -- This ensures the return movement is opposite to the homing movement.
                            ...
                            if log_dir = '1' then current_pos <= current_pos + 1; else current_pos <= current_pos - 1; end if;
                        else tick_cnt <= tick_cnt + 1;
                        end if;
                end case;
            end if;
